...
struct database_view     port_db_view[CRM_MAX_VIEW_DATABASE];
...
/***************************************************************************/
int32_t crm_add_guest_crm_vmport(uint8_t*  mac_p,uint32_t ipaddr,
                           ...
                           uint64_t*  output_crm_port_handle_p)
{
  ...
  do
  {
    ...
    {
      OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"Before appending node to hash table");
    } 
    crm_port_node_p->port_id = 0;   
    			   ...
    			   char*     br_name)
{
  ...
  uint64_t switch_handle,logical_switch_handle,vm_handle,port_handle = 0;
  struct   crm_port* crm_port_node_scan_p = NULL;
  struct   crm_port_notification_data notification_data = {};
  struct   crm_notification_app_hook_info* app_entry_p = NULL;
  uint32_t switch_ip; 
...
{
  int32_t  ret_val;
  uint32_t crm_notification_type = 0;  
  struct   crm_port* crm_port_node_p; 
  ...
  CNTLR_RCU_READ_LOCK_RELEASE(); /*NSML*/
}
/*****************************************************************************************************
...
int32_t crm_add_attribute_to_port(char* port_name_p, struct crm_attribute_name_value_pair* attribute_info_p)
{
  ...
  do
  {
    ...
    crm_create_port_view_entry(port_handle, attribute_info_p->name_string, attribute_info_p->value_string);
...
int32_t crm_delete_attribute_from_port(char* port_name_p, struct crm_attribute_name_value_pair *attribute_info_p)
{
  ...
  OF_LIST_SCAN(port_entry_p->attributes, attribute_entry_p, struct crm_resource_attribute_entry*,lstoffset)
  {
    if(!strcmp(attribute_entry_p->name, attribute_info_p->name_string))
    {
      ...
      crm_remove_port_view_entry(attribute_entry_p->name, attribute_entry_p->value);
      CNTLR_RCU_READ_LOCK_RELEASE();
...

{
  ...
  do
  {
    ...
    if(retval != CRM_SUCCESS)
      break;
    retval = crm_get_exact_attribute(&(port_entry_p->attributes),  attribute_name_string,attribute_output);
  }while(0);
  CNTLR_RCU_READ_LOCK_RELEASE();
  return retval;
}
/****************************************************************************************************/
int32_t crm_create_port_view(char *view_name)
{
  int32_t index = 0, ii;
  uint8_t empty_index_b = 0;

  for(ii = 0;ii<CRM_MAX_VIEW_DATABASE ; ii++)
  {
     if(!strcmp(port_db_view[ii].view , view_name))
     {
        OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"View already created");
        return CRM_FAILURE;
     } 
     else if((empty_index_b == 0) && (!strcmp(port_db_view[ii].view ,"")))    
     {
        empty_index_b = 1;
        index = ii;
     }
  } 
  strcpy(port_db_view[index].view,view_name);
  crm_view_hash_table_init(&port_db_view[index].db_viewtbl_p);
  port_db_view[index].uniqueness_flag = FALSE; //of_view_get_uniqueness(view_name);/*TBD*/
  port_db_view[index].valid_b = TRUE;
...
int32_t crm_create_port_view_entry(uint64_t vm_handle , char* view_name ,char* view_value)
{
  ...
  do
  {
     for(view_index = 0;view_index < CRM_MAX_VIEW_DATABASE ; view_index++)
     {
       if(!strcmp(port_db_view[view_index].view ,view_name))
       {
     ...
     hashkey = crm_get_hashval_byname(view_value,crm_no_of_port_buckets_g);
     CNTLR_RCU_READ_LOCK_TAKE();
     view_node_entry_p = crm_alloc_view_entry_and_set_values(vm_handle, view_name, view_value);
     if (view_node_entry_p == NULL)
     {
       status = CRM_FAILURE;
       break;
     }
     MCHASH_APPEND_NODE(port_db_view[view_index].db_viewtbl_p,hashkey,view_node_entry_p,index,magic,view_node_entry_p,status_b);
     if(FALSE == status_b)
     {
       status = CRM_FAILURE;
       break;
...
int32_t crm_remove_port_view_entry(char* view_name,char* view_value)
{
  struct   database_view_node *view_node_entry_p;
  ...
  do
  {
     for(index = 0;index<CRM_MAX_VIEW_DATABASE;index++)
     {
       if(!strcmp(port_db_view[index].view,view_name))
       {
     ...
     hashkey = crm_get_hashval_byname(view_value,crm_no_of_port_buckets_g);
     CNTLR_RCU_READ_LOCK_TAKE(); 
     MCHASH_BUCKET_SCAN(port_db_view[index].db_viewtbl_p,hashkey, view_node_entry_p,struct database_view_node  *,offset)
     {
          ...
          (!strcmp(view_node_entry_p->node_name, view_name)))
      {
        status_b = FALSE;
        offset = 0;
        MCHASH_DELETE_NODE_BY_REF(port_db_view[index].db_viewtbl_p,view_node_entry_p->index,view_node_entry_p->magic,struct database_view_node *,offset,status_b);
        if (status_b == FALSE)
        {
          status = CRM_FAILURE;
          break;
...
int32_t crm_remove_port_view(char *view_name)
{
  struct   database_view_node *view_node_entry_p;
  int32_t  index , hashkey , offset, ret_val;
  ...
  for(index = 0;index<CRM_MAX_VIEW_DATABASE ; index++)
  {
     if(!strcmp(port_db_view[index].view , view_name))
     {
        OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"View found to deleted");
        node_found_b = 1;
        break;
     }
  }
  if(node_found_b == 0)
  {
    OF_LOG_MSG(OF_LOG_CRM,OF_LOG_ERROR,"Node not found ");
    return CRM_FAILURE;
  }
  ...
  CNTLR_RCU_READ_LOCK_TAKE();
  MCHASH_TABLE_SCAN(port_db_view[index].db_viewtbl_p,hashkey)
  {
    MCHASH_BUCKET_SCAN(port_db_view[index].db_viewtbl_p,hashkey, view_node_entry_p,struct database_view_node  *,offset)
    {
  ...
  if(node_found_b == 1)
  {
     OF_LOG_MSG(OF_LOG_CRM,OF_LOG_ERROR,"Remove view entires to remove the entire view");
     CNTLR_RCU_READ_LOCK_RELEASE();
     return CRM_FAILURE;
  }
  ret_val = crm_view_hash_table_deinit(port_db_view[index].db_viewtbl_p);
  if(ret_val != CRM_SUCCESS)
  {
    OF_LOG_MSG(OF_LOG_CRM,OF_LOG_ERROR,"Hash Table deinit failed");
    CNTLR_RCU_READ_LOCK_RELEASE(); 
    return CRM_FAILURE;
  }
  strcpy(port_db_view[index].view,"");
  port_db_view[index].uniqueness_flag = FALSE; //of_view_get_uniqueness(view_name);/*TBD*/
  port_db_view[index].valid_b = FALSE;
...
int32_t crm_get_first_port_view_entry(char *view_name , struct database_view_node *port_view_node)
{
  int32_t  index;
  uint8_t  node_found_b = 0 ;
  ...
  for(index = 0;index<CRM_MAX_VIEW_DATABASE ; index++)
  {
     if(!strcmp(port_db_view[index].view , view_name))
     {
        OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"View found ");
        node_found_b = 1;
        break;
     }
  }
  ...
  MCHASH_TABLE_SCAN(port_db_view[index].db_viewtbl_p ,hashkey)
  {
    MCHASH_BUCKET_SCAN(port_db_view[index].db_viewtbl_p,hashkey, view_node_entry_p,struct database_view_node  *,offset)
    {
      strcpy(port_view_node->view_value , view_node_entry_p->view_value);
      strcpy(port_view_node->node_name , view_node_entry_p->node_name);
      node_found_b = 1;
      break;
    }
    if(node_found_b)
  ...
  {
     CNTLR_RCU_READ_LOCK_RELEASE();
     OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"Node entry NOT found");
     return CRM_FAILURE;
  }
  CNTLR_RCU_READ_LOCK_RELEASE();
  OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"Node entry found");
  return CRM_SUCCESS;
}

int32_t crm_get_next_port_view_entry(char *view_name , char *view_value ,struct database_view_node *port_view_node)
...
int32_t crm_get_exact_port_view_entry(char *view_name , char *view_value ,struct database_view_node *port_view_node)
...
int32_t crm_get_vmport_by_view(char *view_name , char *view_value ,struct crm_port_config_info *crm_port_info)
{
  ...
  ret_val = crm_get_port_byhandle(view_node_entry_p->node_saferef , &crm_port_node_p);
  if(ret_val != CRM_SUCCESS)
  {
    OF_LOG_MSG(OF_LOG_CRM,OF_LOG_DEBUG,"No port exists");
    CNTLR_RCU_READ_LOCK_RELEASE();
    return CRM_FAILURE;
  } 
  strcpy(crm_port_info->port_name, crm_port_node_p->port_name);
...
int32_t crm_get_nwport_by_view(char *view_name , char *view_value ,struct crm_nwport_config_info *crm_nwport_info)
{
  int32_t  index , ret_val;
  uint8_t  node_found_b = 0 ;
  int32_t  hashkey ,offset ;
